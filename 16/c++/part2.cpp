...
uint64_t tryHard(const uint64_t startX,
                 const uint64_t startY,
    ...
    loop(startX, startY, direction, grid, already);
...
int main() {
    ...
    for (uint64_t index = 0; index < grid.size(); ++index) {
        best = std::max(best, tryHard(0, index, Direction::E));
        best = std::max(best, tryHard(grid[0].size() - 1, index, Direction::W));
    }

    for (uint64_t index = 0; index < grid[0].size(); ++index) {
        best = std::max(best, tryHard(index, 0, Direction::S));
        best = std::max(best, tryHard(index, grid.size() - 1, Direction::N));
    }

    std::cout << "Max number of energized tiles = " << best << std::endl;
}
